...
sized_compact_delta_xy = (jnp.zeros_like(delta_xy).at[:compact_delta_xy.shape[0]].set(compact_delta_xy))
...
class Board(NamedTuple):
    ...
    def valid_spawns_mask(self) -> Array:  # bool[height, width]
        ...
        dxy = jnp.expand_dims(
            jnp.where(
                jnp.expand_dims(self.uses_compact_spawns_mask, (-2, -1)),
                sized_compact_delta_xy,
                delta_xy,
            ),
            -3,
        )  # int[..., 1, 85, 2]
        factory_overlap = (self.factory_pos[..., None, :] + dxy)  # int[..., 2 * MAX_N_FACTORIES, 85, 2]
        ...
        batch_idx = tuple(
            jnp.expand_dims(
                jnp.arange(bs),
                tuple((i for i in range(len(batch_shape) + 1) if i != idx)),
            ) for idx, bs in enumerate(batch_shape))
        return valid_spawns_mask.at[(*batch_idx, factory_overlap[..., 0], factory_overlap[..., 1])].set(False,
                                                                                                        mode="drop")

    @property
    def old_valid_spawns_mask(self) -> Array:
        valid_spawns_mask = ~self.map.ice & ~self.map.ore  # bool[..., height, width]
        valid_spawns_mask = valid_spawns_mask & jnp.roll(valid_spawns_mask, 1, axis=-2)
        valid_spawns_mask = valid_spawns_mask & jnp.roll(valid_spawns_mask, -1, axis=-2)
        valid_spawns_mask = valid_spawns_mask & jnp.roll(valid_spawns_mask, 1, axis=-1)
        ...
        factory_overlap = (self.factory_pos[..., None, :] + compact_delta_xy)  # int[..., 2 * MAX_N_FACTORIES, 49, 2]
        factory_overlap = factory_overlap.reshape(batch_shape + (-1, 2))  # int[..., 2 * MAX_N_FACTORIES * 49, 2]
        factory_overlap = jnp.clip(factory_overlap, 0, jnp.array([self.height - 1, self.width - 1]))
    ...
    def new(
        ...
        return cls(
            ...
            uses_compact_spawns_mask=env_cfg.USES_COMPACT_SPAWNS_MASK,
        )
    ...
    def from_lux(cls: Type["Board"], lux_board: LuxBoard, buf_cfg: JuxBufferConfig) -> "Board":
        ...
        factory_occupancy_map = jnp.array(
            lux_board.factory_occupancy_map,
    ...
    def __eq__(self, __o: "Board") -> bool:
        if not isinstance(__o, Board):
            return False
        return ((self.height == __o.height)
                & (self.width == __o.width)
                & (self.seed == __o.seed)
                & (self.factories_per_team == __o.factories_per_team)
                & (self.map == __o.map)
    ...
    def update_factories_map(self, factories) -> "Board":
        ...
        factory_occupancy_map = jnp.full_like(
            self.factory_occupancy_map,
